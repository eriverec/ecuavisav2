...
$(document).ready(function () {
  ...
  $(
    ".HOME_MENU_RD .img-header-prog .mediaBlock #rd_cont_botones .btn-gye"
  ).hide();
  $(
    ".HOME_MENU_RD .img-header-prog .mediaBlock #rd_cont_botones .btn-quito"
  ).hide();
  ...
  function activePlayer() {
    ...
    $("#playerembed").removeClass("d-none");
  ...
  if (day > 0 && day <= 5) {
    ...
    if ("05:55" <= timeOfDay && timeOfDay <= "06:55") {
      ...
      $(".title_programa").append("Televistazo Al Amanecer");
    } else if ("06:55" <= timeOfDay && timeOfDay <= "07:30") {
      activeBlock(2);
      activePlayer();
      $(".title_programa").append("Contacto Directo");
    } else if ("07:30" <= timeOfDay && timeOfDay <= "09:00") {
      activeBlock(3);
      activePlayer();
      showTelevistazoAmanecerComunidad();
      $(".title_programa").append("Televistazo en la comunidad");
    } else if ("10:30" <= timeOfDay && timeOfDay <= "13:00") {
      activeBlock(4);
      activePlayer();
      $(".title_programa").append("En Contacto");
    } else if ("13:00" <= timeOfDay && timeOfDay <= "14:00") {
      activeBlock(5);
      activePlayer();
      $(".title_programa").append("Televistazo 13h00");
