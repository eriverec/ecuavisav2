<script setup>
  ...
  async function getDrivers(fechai = '', fechaf = '') {
    ...
    await fetch('https://servicio-de-actividad.vercel.app/actividad/seccion/full/Entretenimiento', requestOptions)
      ...
      .catch(error => { 
        configSnackbar.value = {
            message: "No se pudo recuperar los registros de entretenimiento, por favor intente nuevamente.",
            type: "error",
            timeout: 4000,
            model: true
        };
  ...
  async function downloadSelection() {
    ...
    logAction("export-notasEntretenimiento"); 
    /*
    await accionBackoffice({
     "usuario": userBackoffice.value.email,   
     "pagina": "trazabilidad-visitas-list-notasEntretenimiento",
