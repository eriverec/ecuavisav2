    ...
    await fetch('https://servicio-de-actividad.vercel.app/actividad/seccion/full/Noticias', requestOptions)
      ...
      .catch(error => {
        configSnackbar.value = {
            message: "No se pudo recuperar los registros de noticias, por favor intente nuevamente.",
            type: "error",
            timeout: 4000,
            model: true
        };
...
<style type="text/css">
  .timeline-2 .v-timeline-divider__after {
    display: none;
}
.timeline-2 .v-timeline-divider__dot{
  background:none;
}
</style>
