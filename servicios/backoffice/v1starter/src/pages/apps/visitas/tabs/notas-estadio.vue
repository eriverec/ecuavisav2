<script setup>
  ...
  async function getDrivers(fechai = '', fechaf = '') {
    ...
    await fetch('https://servicio-de-actividad.vercel.app/actividad/seccion/full/Estadio', requestOptions)
      ...
      .catch(error => { 
        configSnackbar.value = {
            message: "No se pudo recuperar los datos de registros para estadio, por favor intente nuevamente.",
            type: "error",
            timeout: 4000,
            model: true
        };
  ...
  onMounted(async() => {
    ...
    await accionBackoffice({
              ...
              "pagina": "trazabilidad-visitas-list-notasEstadio",
  ...
  async function downloadSelection() {
    ...
    let title = "estadio_ultimos_usuarios_" + titleSelected.value.replace(/[^A-Z0-9]+/ig, "_");
    //console.log("doc", doc);
    //if(usersFull.length > totalUsers){
    logAction("export-notasEstadio");  
    /*
    await accionBackoffice({
     "usuario": userBackoffice.value.email,   
     "pagina": "trazabilidad-visitas-list-notasEstadio",
