<script setup>
  ...
  async function getDrivers(fechai = '', fechaf = '') {
    ...
    await fetch('https://servicio-de-actividad.vercel.app/actividad/seccion/full/Lo-ultimo', requestOptions)
      ...
      .catch(error => { 
        configSnackbar.value = {
            message: "No se pudo recuperar los registros de lo último, por favor intente nuevamente.",
            type: "error",
            timeout: 4000,
            model: true
        };
  ...
  onMounted(async() => {
    ...
    await accionBackoffice({
              ...
              "pagina": "trazabilidad-visitas-list-notasLoultimo",
  ...
  async function downloadSelection() {
    ...
    let title = "lo_ultimo_ultimos_usuarios_" + titleSelected.value.replace(/[^A-Z0-9]+/ig, "_");
    //console.log("doc", doc);
    //if(usersFull.length > totalUsers){
    logAction("export-notasLoultimo");  
    /*
    await accionBackoffice({
     "usuario": userBackoffice.value.email,   
     "pagina": "trazabilidad-visitas-list-notasLoultimo",
