<script setup>
  ...
  const resolveUltimosUsuarios = async (title) => {
    // fecha.value.i.format("MM/DD/YYYY")
    ...
    await fetch('https://servicio-de-actividad.vercel.app/backoffice/visitas/paginas/title/'+title+'?fechaf='+fecha.value.f.format("MM/DD/YYYY")+'&fechai='+fecha.value.i.format("MM/DD/YYYY")+'&page=1&limit=500')
      ...
      .then(async response => {
        ...
        for (var i in array) {
          const fullFecha = moment(array[i].timestamp, "DD/MM/YYYY, HH:mm:ss").format('DD/MM/YYYY HH:mm:ss') == 'Invalid date' ? moment(array[i].timestamp, "MM/DD/YYYY, HH:mm:ss").format('DD/MM/YYYY HH:mm:ss'):moment(array[i].timestamp, "DD/MM/YYYY, HH:mm:ss").format('DD/MM/YYYY HH:mm:ss');
          const fecha_temp = moment(array[i].timestamp.split(",")[0], "DD/MM/YYYY").format('DD/MM/YYYY') == 'Invalid date' ? moment(array[i].timestamp.split(",")[0], "MM/DD/YYYY").format('DD/MM/YYYY'):moment(array[i].timestamp.split(",")[0], "DD/MM/YYYY").format('DD/MM/YYYY');
          // console.log(array[i], fullFecha)
          let data = {
            first_name:array[i].users.first_name, 
            last_name:array[i].users.last_name, 
            userId:array[i].userId, 
            fecha: fecha_temp, 
            fechaRaw: fecha_temp, 
            hora:array[i].timestamp.split(",")[1].trim(), 
            horaRaw:array[i].timestamp.split(",")[1].trim(), 
            fullFecha, 
    ...
    let resultado = Object.values(arrayFiltro);
    ...
    ultimosUsuarios.value = resultado;
    ultimosUsuarios.value = ultimosUsuarios.value.slice(0, 25);
  ...
  const resolveUltimasVisitasUser = async (user = {}) => {
    ...
    .then(async response => {
      ...
      for (var i in array) {
        const fullFecha = moment(array[i].fullTime, "DD/MM/YYYY, HH:mm:ss").format('DD/MM/YYYY HH:mm:ss') == 'Invalid date' ? moment(array[i].fullTime, "MM/DD/YYYY, HH:mm:ss").format('DD/MM/YYYY HH:mm:ss'):moment(array[i].fullTime, "DD/MM/YYYY, HH:mm:ss").format('DD/MM/YYYY HH:mm:ss');
        const fecha_temp = moment(array[i].fullTime.split(",")[0], "DD/MM/YYYY").format('DD/MM/YYYY') == 'Invalid date' ? moment(array[i].fullTime.split(",")[0], "MM/DD/YYYY").format('DD/MM/YYYY'):moment(array[i].fullTime.split(",")[0], "DD/MM/YYYY").format('DD/MM/YYYY');
        ...
        const data = {
          ...
          fechaRaw: fecha_temp,
          fullFecha: fullFecha,
          hora: (array[i].fullTime.split(" ")[1] || "").trim()
        }
