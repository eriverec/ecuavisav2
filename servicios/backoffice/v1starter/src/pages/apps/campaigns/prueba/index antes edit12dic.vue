...
const openModal = async () => {
  try {
    ...
    const payload = {
      campaignTitle: nombreCampania.value || `Temporal-${Date.now()}`,
      description: descripcionCampania.value || "Temporal",
      userIdRemove: [],
      userIdAdd: [],
      statusCampaign: true,
      urls: {
      ...
      participantes: "Todos",
      otroValor: null,
      type: languages.value || "html",
      coleccion: criterio.value.length > 0 ? criterio.value.join(',') : "dispositivos",
      position: posicion.value?.[0] || "RDTop1",
      campaignSlug: `temp-${Date.now()}`
    };
    ...
    if (data.resp && data._id) {
      tempCampaignId.value = data._id;
    ...
    // Aquí podrías agregar un manejo de error visual
  } finally {
...
// Agregar computed para controlar el estado de bloqueo
const isPersonalizadoDisabled = computed(() => {
  return criterio.value.length > 0;
});
watch(() => selectedItem.value, async (newValue) => {
  if (newValue) {
    selectedItemCiudad.value = [];
    cityList.value = ['Todas las ciudades']; // Agregar opción por defecto
...
// Watch para el manejo de ciudades
watch(() => selectedItem.value, (newValue) => {
  if (newValue) {
    selectedItemCiudad.value = [];
    const selectedCountry = dataCountry.value.find(country => country.country === newValue);
    if (selectedCountry) {
      cityList.value = ['Todas las ciudades', ...selectedCountry.data.map(city => city.city)];
    } else {
      cityList.value = ['Todas las ciudades'];
    }
  } else {
    cityList.value = [];
    selectedItemCiudad.value = [];
...
// Ajuste para el watch de selección de ciudad y actualización de usuarios
watch(() => selectedItemCiudad.value, async (newValue) => {
  if (newValue) {
    await getUsuarios();
  }
}, { deep: true });
...
onMounted(async () => {
  ...
  try {
    ...
    const raw = JSON.stringify({
      ...
      "ciudad": (selectedItemCiudad.value?.includes('Todas las ciudades')) ? 
        -1 : 
        (selectedItemCiudad.value?.length > 0) ? selectedItemCiudad.value : -1,
    ...
    // Obtenemos los usuarios totales y sus IDs
    ...
    console.log("Respuesta completa de usuarios:", datos);

    if (datos.resp && datos.userIds) {
      dataUsuarios.value = {
        total: datos.total || 0,
        userIds: datos.userIds // Guardamos el array de IDs
      };
      console.log("Total de usuarios:", dataUsuarios.value.total);
      console.log("IDs guardados:", dataUsuarios.value.userIds);
    } else {
      throw new Error('No se recibieron IDs de usuarios');
    }
  ...
  try {
    ...
    // Asegurarnos de tener los usuarios actualizados
    await getUsuarios();
    
    let tipoParticipantes = "Todos";
    if (criterio.value.length > 0) {
      tipoParticipantes = "Filtrado";
    } else if (selectItemParticipantes.value === 'Usuarios específicos') {
      tipoParticipantes = "Usuarios específicos";
    }
    ...
    console.log('JSON a enviar con IDs:', JSON.stringify(jsonEnviar, null, 2));
