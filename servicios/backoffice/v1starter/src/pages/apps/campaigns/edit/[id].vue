...
import 'vue3-form-wizard/dist/style.css';
  ...
  async function updateCampaign(jsonEnviar){
    try {
  ...
  async function onComplete() {
    ...
    try{
      ...
      var respCampaign = await updateCampaign(jsonEnviar);
  ...
  async function readRecord() {
    ...
    try {
      const response = await fetch(`https://ads-service.vercel.app/campaign/${route.params.id}/`);
  ...
  const getUserIds = async (id, page = 1, limit = 10000) => {
    try{
      ...
      var response = await fetch(`https://ads-service.vercel.app/campaign/${id}/user-ids?limit=${limit}&page=${page}&details=true`, requestOptions);
      const data = await response.json();
      if(data && data.length > 0){
        return procesarListas(data[0].userId, data[0].userIdAdd, data[0].userIdRemove);
      }else{
        return [];
      };
    }catch(error){
  ...
  async function procesarListas(userId, userIdAdd, userIdRemove) {
    // Unir las listas y eliminar duplicados
    const listaUnida = [...new Set([...userId, ...userIdAdd])];

    // Obtener una lista de wylexId presentes en userIdRemove
    const idsToRemove = userIdRemove.map((user) => user.wylexId);

    // Filtrar la lista para descartar los objetos con wylexId presentes en userIdRemove
    const listaFinal = listaUnida.filter((user) => !idsToRemove.includes(user.wylexId));
  ...
  onMounted(async () => {
    ...
    if(!cloneCampaign){
      snackbar.value = {
        show: true,
        text: 'Error al cargar la campaña o no existe',
        color: 'error',
      }
      return null;
    }
    ...
    if(modoPersonalizado.value){
      // Cargar los IDs de los usuarios
        let skip = 1;
        let batchSize = 20000;
        let dataFull = [];
        ...
        while (true) {
          loadingPanel.value=true;
          const batchRegister = await getUserIds(route.params.id, skip, batchSize);
        ...
        cloneCampaign.userId = dataFull;

        const validUsers = cloneCampaign?.userId.filter(row => row.wylexId).map(row => ({
          wylexId: parseInt(row.wylexId),
          firstname: row.firstname,
          last_name: row.last_name,
          email: row.email
        })).filter(user => !isNaN(user.wylexId))
      ...
      dataUsuarios.value = {total: cloneCampaign?.userIdSize, userIds: [] };
    }
