...
async function loadSiteNames() {
  try {
    ...
    // Extract and process site names with validation
    const siteNames = mediaData
      .filter(media => media && media.media_communication) // Filtrar elementos nulos o sin media_communication
      .map(media => {
        const siteName = media.media_communication.toUpperCase();
        // Add any special case handling if needed
        switch (siteName) {
          case 'TC':
    ...
    // Update the itemsSitioWeb ref with unique values
    itemsSitioWeb.value = [...new Set(siteNames)].filter(Boolean); // Eliminar valores nulos o undefined
    
    console.log('Sitios web cargados:', itemsSitioWeb.value); // Para debugging
  } catch (error) {
    console.error('Error loading site names:', error);
    itemsSitioWeb.value = []; // Establecer un valor por defecto en caso de error
  }
}
...
const principalData = async function () {
  try {
    ...
    const response = await fetch('https://services.ecuavisa.com/gestor/competencias/scrappin/dinamico/config.php?api=all');
    const rawMediaData = await response.json();

    mediaData.value = rawMediaData;

    // Process all articles from all media sources
    const processedArticles = rawMediaData.reduce((acc, media) => {
      if (!media || !Array.isArray(media.articles)) return acc;

      const articles = (media.articles || []).map(article => ({
        ...article,
        sitio: media.media_communication.toUpperCase(),
        color: availableColors[acc.length % availableColors.length],
        vertical: article.seccion || 'Home',
        subVertical: article.subseccion || 'Sin Categorizar',
        fechaPublicacion: article.fechaPublicacion || getDefaultDate().format("DD/MM/YYYY HH:mm:ss"),
        enlace: article.link,
        titulo: article.title,
        picture: article.image,
        keywords: article.tags || article.keywords || '',
        tipo: article.tipo || 'Article'
      }));
      return [...acc, ...articles];
    }, []);
    ...
    const uniqueArticles = Array.from(
      new Map(processedArticles.map(item => [item.enlace, item])).values()
    );
    ...
    data.value = sortedArticles;
    
    // Update last update timestamp
    if (sortedArticles.length > 0) {
      const latestMedia = rawMediaData.reduce((latest, media) => {
        const mediaUpdate = moment(media.last_update, "YYYY-MM-DD HH:mm:ss");
        return !latest || mediaUpdate.isAfter(latest) ? mediaUpdate : latest;
      }, null);
      
      lastUpdate.value = latestMedia.format("DD/MM/YYYY HH:mm");
    }
        ...
        <VCard>
          <VCardItem>
            <div class="d-flex content-title flex-wrap w-100">
              ...
              <div class="w-100 mt-4 flex justify-center" v-if="mediaData.length > 0">
