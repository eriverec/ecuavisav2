...
const generateWordCloud = () => {
  ...
  WordCloud(canvas, {
    list: words.value.map(d => [d.text, d.size]), // Preparamos los datos para la librería
    gridSize: 15,
    weightFactor: 3,
    ...
    rotateRatio: 0.5, // Rotar algunas palabras
    minSize: 20,
    ...
    hover: (item, dimension) => {
      if (item) {
        canvas.style.cursor = "pointer"; // Cambia el cursor cuando pasa por encima
      } else {
        canvas.style.cursor = "default"; // Vuelve a la normalidad
      }
    }
  });
};
