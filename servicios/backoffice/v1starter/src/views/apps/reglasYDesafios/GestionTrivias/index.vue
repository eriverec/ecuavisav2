    ...
    <section>
        ...
        <VRow>
            <VCol cols="12" sm="12" lg="12">
                    ...
                    <VCardItem v-else class="px-0">
                                ...
                                <tr v-for="item in paginatedTrivias">
                                    ...
                                    <td class="text-medium-emphasis">
                                        <VBtn variant="text" icon
                                            @click="mostrarRespuestasUsuarios(item._id, item.nombre)">
                        ...
                        <div class="d-flex align-center justify-space-between botonescurrentPage">
                            <VBtn icon="tabler-arrow-big-left-lines" @click="prevPage" :disabled="currentPage === 1">
                            </VBtn>
                            Página {{ currentPage }}
            ...
            <VCol v-if="respuestasUsuariosVisible" cols="12" sm="12" lg="12">
                    ...
                    <VCardTitle class="px-0 pt-4 ">Respuestas de {{ nombreTriviaSelected }}</VCardTitle>

                    <VCardItem class="px-0" v-if="respuestasUsuarios.length > 0">
                        <div class="respuesta-card mb-4 w-100" v-for="(item, index) in respuestasUsuarios">
                            <div class="d-flex align-center gap-4 mt-6 ml-4 mb-2">
                                ...
                                <div>
                                    <VBtn v-if="!nombreUsuarioVisible.includes(index)" variant="text" icon
                                        @click="mostrarNombreUsuario(item.idUsuario, index)">
                                        <VIcon size="22" icon="tabler-eye" />
                                    </VBtn>
                        ...
                        <VRow class="d-flex flex-wrap justify-center gap-4">
                            <VRow>
                                ...
                                <VCol cols="6" class="d-flex">
                                    ...
                                    <VBtn class="mr-auto" color="primary" prepend-icon="tabler-plus" variant="tonal"
                                        @click="addRegla">
                                        Crear regla
                                    </VBtn>
                                ...
                                <VRow v-if="reglaFormVisible" class="d-flex my-4">
                                    ...
                                    <VCol cols="6">
                                        <VTextField v-model="frecuenciaValor" label="Frecuencia de valor"
                                            type="number" />
                                    </VCol>
                                    ...
                                    <VCol cols="12">
                                        <VSelect v-model="categoriaDesafio" :items="categoriaItems"
                                            label="Categoría del desafío" />
                                    </VCol>

                                    <VCol cols="12">
                                        <VSelect v-model="tipoModel" :items="tipoItemsDesafio"
                                            label="Tipos del desafío" />
                                    </VCol>
                                ...
                                <VCol cols="12" class="d-flex">
                                    ...
                                    <VBtn class="ml-auto" color="primary" prepend-icon="tabler-plus" variant="tonal"
                                        @click="resolveAñadirPregunta">
                                        Añadir pregunta
                                    </VBtn>
                                ...
                                <div v-for="(p, index) in preguntas" cols="12" class="w-100 my-4 item-cards">
                                    <VBtn v-if="preguntas.length > 1" class="ml-auto boton-eleminar-itemsCards"
                                        size="38" color="error" @click="eliminarPregunta(index)">
                                        <VIcon icon="tabler-x" size="22" />
                                    </VBtn>

                                    <VCardText>
                                        <VCol cols="12">
                                            <VTextField class="mt-2" v-model="p.pregunta" label="Pregunta"
                                                placeholder="Escriba la pregunta" />
                                        </VCol>

                                        <VCol cols="12">
                                            <VSelect class="mt-2" v-model="p.tipo" label="Tipo" :items="tipoItems"
                                                @update:model-value="resolveOpciones(index, p.tipo)" />
                                        </VCol>

                                        <VCol cols="12">
                                            <VSelect class="mt-2" v-model="p.tipoSelector" label="Tipo de selector"
                                                :items="tipoSelectorItems" />
                                        </VCol>

                                        <VCol cols="12" v-if="p.tipo != 'votacion'">
                                            <VTextField :class="p.tipo == 'opciones' ? 'mt-2' : 'my-2'" v-model="p.respuesta"
                                                label="Respuesta" placeholder="Escriba la respuesta" />
                                        </VCol>
                                        ...
                                        <VCol v-if="p.tipo == 'opciones' || p.tipo == 'votacion'" cols="12"
                                            ...
                                            <VBtn class="ml-auto" color="primary" prepend-icon="tabler-plus"
                                                variant="tonal" @click="resolveAñadirOpcion(index)">
                                                Añadir opción
                                            </VBtn>

                                        </VCol>
                                        <VDivider
                                            v-if="(p.tipo == 'opciones' || p.tipo == 'votacion') && p.opciones.length > 0" />

                                        <div v-if="p.tipo == 'opciones' || p.tipo == 'votacion'"
                                            v-for="(o, index1) in p.opciones" cols="12">

                                            <VCardText class="text-center ml-4 my-4">
                                                <VRow>
                                                    <VCol cols="8">
                                                        <VTextField v-model="p.opciones[index1]"
                                                            :label="'Opción ' + (index1 + 1)"
                                                            placeholder="Escriba la opción" />
                                                    </VCol>
                                                    <VCol cols="4" v-if="p.opciones.length > 2">
                                                        <VBtn size="38" color="error"
                                                            @click="eliminarOpcion(index, index1)">
                                                            <VIcon icon="tabler-x" size="22" />
                                                        </VBtn>
                                                    </VCol>
                                                </VRow>
