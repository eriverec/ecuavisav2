...
class SendPulse
{
	...
	private function getDetallesNotaPrincial($list, $getNota, $idarticle)
	{
		...
		if (is_array($list)) {
			...
			foreach ($list as $key => $value) {
				...
				/*if ($count < 5 && isset($value->name) && $value->name === 'Text') {
					$content[] = '
    ...
    // array_splice($arr, $orden, 0, array($itemToMove));
		// Ajustar el orden según el valor de $name
    // switch ($name) {
		// 	case 'Headline':
		// 			array_splice($arr, $orden, 0, array($itemToMove));
		// 			break;
		// 	case 'Text':
		// 			array_splice($arr, $orden + 1, 0, array($itemToMove)); // Se coloca después de Headline
		// 			break;
		// 	case 'Image':
		// 			array_splice($arr, $orden + 2, 0, array($itemToMove)); // Se coloca después de Text
		// 			break;
		// }
		// Ajustar el orden según el valor de $name, excepto para 'Image'
    if ($name !== 'Image') {
			array_splice($arr, $orden, 0, array($itemToMove));
		} else {
				$arr[] = $itemToMove; // Añadir al final del arreglo
		}
