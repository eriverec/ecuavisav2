...
jQryIter('.IMG_TIT_TXT.mult').each(function (i, obj) {
  jQryIter(this).find('.Multimedia_Text').css('display', 'none'); // ocultamos players
  //obtenemos variables
  const mp3url = jQryIter(this).find('source').attr('src');
  const title_pd = jQryIter(this).find('h2').text();

  // agregamos botones del player
  jQryIter(this).find('.media_block .pod_icon_play').append(`
    ...
    <path class="play-stop-button__play" d="M13.0273 23.1822V12.3681C13.0273 11.5599 13.9364 11.0856 14.5993 11.5479L24.7893 18.6537C25.3633 19.0539 25.3593 19.9047 24.7817 20.2995L13.0273 28.335" stroke="#2927B9" stroke-width="2" stroke-linecap="round" /></svg>`);
});
...
function podcastPlay() {
  jQryIter(document).ready(function () {
    ...
    if (event.currentTarget.classList.contains('play-stop-button--playing')) {
      setTimeout(function () {
        jQryIter('.mejs__playpause-button').click();
      }, 100);
    } else {
      playStop();
    }
    ...
    jQryIter('.mainplayer').attr('src', theMp3);
    jQryIter('.podcast__episode_title').html(theTitle);
    jQryIter('.mejs__playpause-button').click();
